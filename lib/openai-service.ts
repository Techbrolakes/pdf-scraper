...
const RESUME_SCHEMA = {
  ...
  properties: {
    profile: {
      ...
      required: ['name', 'surname', 'email', 'headline', 'professionalSummary', 'linkedIn', 'website', 'country', 'city', 'relocation', 'remote'],
    ...
    workExperiences: {
      ...
      items: {
        ...
        required: ['jobTitle', 'employmentType', 'locationType', 'company', 'startMonth', 'startYear', 'endMonth', 'endYear', 'current', 'description'],
        additionalProperties: false,
      },
    },
    educations: {
      ...
      items: {
        ...
        properties: {
          school: { type: 'string' },
          degree: { type: 'string', enum: ['HIGH_SCHOOL', 'ASSOCIATE', 'BACHELOR', 'MASTER', 'DOCTORATE'] },
  ...
  required: ['profile', 'workExperiences', 'educations', 'skills', 'licenses', 'languages', 'achievements', 'publications', 'honors'],
  additionalProperties: false,
}
 ...
 */
export async function extractResumeFromImages(imageUrls: string[]): Promise<ResumeData> {
  try {
...
export function validateResumeData(data: unknown): data is ResumeData {
  ...
  if (!resume.profile || !Array.isArray(resume.workExperiences) || !Array.isArray(resume.educations)) {
    return false
  }
  
  // Basic profile validation
  if (typeof resume.profile !== 'object') return false
